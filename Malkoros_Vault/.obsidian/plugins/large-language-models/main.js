...
async function createVectorAndUpdate(files, assistant2, OpenAI_API_Key, fileSystem) {
  ...
  const file_ids = await Promise.all(
    files.map(async (filePath) => {
      ...
      const fileToUpload = await toFile2(new Blob([fileContent]), filePath);
...
var ChatContainer = class {
  ...
  async handleGenerateClick(header, sendButton) {
    ...
    const refreshButton = this.historyMessages.querySelector(".llm-refresh-output");
    refreshButton == null ? void 0 : refreshButton.remove();
    const { model, modelName, modelType, endpointURL, modelEndpoint } = getViewInfo(this.plugin, this.viewType);
  ...
  setMessages(replaceChatHistory = false) {
    ...
    const settingType = settings[this.viewType];
  ...
  createMessage(content, index, finalMessage) {
  ...
  generateIMLikeMessgaes(messages2) {
    let finalMessage = false;
    messages2.map(({ content }, index) => {
...
var HistoryContainer = class {
  ...
  displayNoHistoryView(parentElement) {
    ...
    createChatButton.onClick(() => {
      ...
      const chatContainer = document.querySelector(`[class*="${prefix}-chat-container"]`);
      chatContainer.show();
  ...
  generateHistoryContainer(parentElement, history, containerToShow, chat2, Header2) {
    ...
    const eventListener = () => {
      ...
      chat2.generateIMLikeMessgaes(messages2);
...
var FAB = class {
  ...
  generateFAB() {
    ...
    button.setIcon("message-circle").setClass("buttonItem").onClick(() => {
      if (!viewArea.isShown()) {
